const AddValidation = (() => {
    ...
    const pwd = (form) => {
        const re = /.{3,}/
        if (!/\S+/.test(form.pwd.value)) form.pwd.setCustomValidity("Password can't be empty!")
        else if (!re.test(form.pwd.value)) form.pwd.setCustomValidity("Password must be at least 3 characters!")
        else form.pwd.setCustomValidity("")
    }

    const pwdConfirm = (form) => {
        const re = /\S+/
        if (!re.test(form['pwd-confirm'].value)) form['pwd-confirm'].setCustomValidity("Confirm Password can't be empty!")
        else if (form['pwd-confirm'].value != form.pwd.value) form['pwd-confirm'].setCustomValidity("Passwords do not match!")
